...
const projects = [
  {
    ...
    bentoClass: 'md:col-span-2 h-[450px]',
  },
  {
    ...
    bentoClass: 'md:col-span-1 h-[450px]',
  },
  {
    ...
    <section id="projects" className="py-24 relative overflow-hidden bg-[#050508]">
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        >
          {projects.map((proj, idx) => {
            ...
            return (
              <motion.div
                ...
                className={`glass-card rounded-3xl border border-slate-900 overflow-hidden flex flex-col group hover:border-flutter/35 transition-all duration-300 relative ${proj.bentoClass}`}
                style={{
              ...
              >
                ...
                <div className="absolute top-6 right-6 z-10 w-10 h-10 rounded-xl bg-slate-950/80 border border-slate-800/80 flex items-center justify-center text-slate-400">
                  <Icon className="w-5 h-5 text-flutter-cyan" />
                </div>

                {/* Card Main Wrapper */}
                <div className="p-8 flex flex-col justify-end h-full z-10 relative">
                  
                  {/* Default State: Title & Tags */}
                  <div className="space-y-4">
                    <h3 className="text-2xl font-black text-white group-hover:text-flutter-cyan transition-colors leading-tight">
                      {proj.title}
                    </h3>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {proj.tech.map((t, tIdx) => (
                        <span
                          key={tIdx}
                          className="px-3 py-1 rounded-xl bg-slate-950/80 border border-slate-800/60 text-[10px] text-slate-300 font-extrabold uppercase"
                        >
                          {t}
                        </span>
                      ))}
                  ...
                  {/* Hover State: Reveal Animation Overlay using AnimatePresence */}
                  <div className="overflow-hidden">
                    <motion.div
                      ...
                      className="space-y-4 pt-4 border-t border-slate-900/60 mt-4"
                    >
                      ...
                      {/* Highlights */}
                      <div className="p-3 rounded-xl bg-slate-950/70 border border-slate-900 flex items-start gap-2.5">
                        <Cpu className="w-4 h-4 text-flutter-cyan shrink-0 mt-0.5" />
                        <div className="space-y-0.5">
                          <span className="text-[9px] text-slate-500 uppercase tracking-widest font-extrabold block">Architecture Highlight</span>
                      ...
                      <div className="flex items-center justify-between pt-2">
                        <a
                          ...
                          className="inline-flex items-center gap-2 text-xs font-extrabold text-slate-400 hover:text-white transition-colors"
                        >
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-1.5 text-xs font-extrabold text-flutter-cyan hover:text-white transition-colors animate-pulse"
                        >
                          <span>Preview</span>
          ...
          {/* Custom Bento Card: DevOps Dashboard Metric (Row 2, Spanning 2 cols) */}
          <motion.div
            variants={cardVariants}
            className="glass-card rounded-3xl border border-slate-900 p-8 flex flex-col md:col-span-2 h-[450px] relative overflow-hidden group hover:border-flutter/25 transition-all duration-300 shadow-[0_10px_30px_-15px_rgba(245,158,11,0.06)]"
          >
            ...
            <div className="flex items-center justify-between mb-6 border-b border-slate-900 pb-4 relative z-10">
              ...
              <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[10px] font-bold uppercase tracking-wider animate-pulse">
                <span>All Pipelines Passing</span>
              </div>
            </div>

            {/* Pipeline Visual representation */}
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 flex-grow items-center relative z-10">
              {[
                ...
                <div key={idx} className="bg-slate-950/70 border border-slate-900 rounded-2xl p-4 flex flex-col justify-between h-44 shadow-md group-hover:border-slate-800/50 transition-colors">
